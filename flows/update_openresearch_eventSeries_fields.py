...
from tasks.utils import extract_event_fields_from_wikitext, update_dict
...
def build_eventSeries_fields_extraction_prompt(
    ...
    - title: primary name of the event series (e.g., conference title)
...
@flow(name="update-openresearch-eventSeries-fields", description="Flow to update OpenResearch event pages with new edition fields using LLM extraction")
def update_openresearch_eventSeries_fields(
    api_url: str,
    username: str,
    password: str,
    ...
    for series in titles[0:5]:
        ...
        if series_title:
            ...
            logger.info(f"{series}: [SKIP] No title found in event or series")
            continue
        
        event_wikitext = get_page_wikitext(api_url, series, session)
        tpl = find_template_block(event_wikitext, template_name)
        if not tpl:
            logger.info(f"{series}: [SKIP] No {template_name} template")
            continue
        
        tpl_text, _, _ = tpl
        
        extracted_fields = extract_fields_from_template(tpl_text, fields)
        logger.debug(f"{series}: Extracted fields: {extracted_fields}")
        
        if None in extracted_fields.values():
            logger.info(f"{series}: [INFO] Missing fields detected, attempting LLM extraction")
            try:
                if llm_api_key:
                    llm_prompt = build_eventSeries_fields_extraction_prompt(
                        title=title,
                        acronym=series,
                        fields=fields
                    )
                    logger.debug("LLM prompt for %s (%s): \n %s", title, series, llm_prompt)
                    parsed_response = get_event_fields(llm_api_key, llm_prompt)
                    logger.debug("LLM output for %s (%s): \n %s", title, series, parsed_response)
                    if parsed_response:
                        update_dict(extracted_fields, parsed_response)
                        logger.debug("Updated fields after LLM extraction for %s (%s): \n %s", title, series, extracted_fields)
                        ...
                        if not changed:
                            logger.info(f"[UNCHANGED] {series} already has desired params.")
                            continue
                        summary = f"Set {parameter_to_set} (acronym={series})"
                        logger.debug(f"New wikitext for {series}:\n{new_wikitext}")
                        logger.info(f"Editing page {series} with text: {new_wikitext}")
                        res = edit_page(api_url, series, new_wikitext, csrf_token, session, summary, dry_run)
                        logger.info("Edit result for title %s: result: %s", series, res['error']['code'] if res.get('error') else res)
                        if res.get('error'):
                            logger.error("Edit result for title %s: result: %s", series, res['error']['code'])
            except Exception as e:
                logger.error("Exception for %s: %s", series, e)
                # time.sleep(1)
    return True
...
if __name__ == "__main__":
    ...
    update_openresearch_eventSeries_fields(API, USER, PASS, 
                                      core_26_dict, core_23_dict,
                                      fields="Homepage:str, Confidence:str, Sources:list",
                                      dry_run=False, 
