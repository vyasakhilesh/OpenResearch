...
@task(name="create-stats-openresearch-eventSeries", description="collecting statistics of OpenResearch event series pages in a dataframe")
def create_stats_openresearch_eventSeries(
    api_url: str,
    username: str,
    password: str,
    core_all_details_path: str,
    ...
    df_stats_openresearch_eventSeries = pd.DataFrame()
    for idx, page_title in enumerate(page_titles):  # limit to first 10 for testing
        logger.info(f"Processing page {idx}:{page_title}")
        try:
            event_wikitext = get_page_wikitext(api_url, page_title, session)
            df_stats_openresearch_eventSeries = append_eventSeries_template_to_df(event_wikitext, df_stats_openresearch_eventSeries)
        except Exception as e:
            logger.error("Get Event Series Wikitext Exception for %s: %s", page_title, e)
            continue
